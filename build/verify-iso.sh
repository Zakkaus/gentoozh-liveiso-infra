...
leak=""
for p in "${R}"/usr/local/bin/gigos-*.sh "${R}"/etc/systemd/system/gigos-*.service; do
    [ -e "${p}" ] || continue
    b="$(basename "${p}")"
    echo "${b}" | grep -qE "${KEEP_RE}" && continue
    grep -qF "${b}" "${R}"/etc/calamares/modules/shellprocess*.conf 2>/dev/null || leak="${leak} ${b}"
done
if [ -n "${SHP_C}" ] && [ -z "${leak}" ]; then
    ok "live 专用 gigos 服务/脚本都在装机清理名单(不会泄漏进装好系统)"
elif [ -z "${SHP_C}" ]; then
    bad "找不到 shellprocess*.conf,无法核对 live 服务清理(=可能泄漏)"
else
    bad "live 专用项未被装机清理(会泄漏进装好系统=后门):${leak}"
fi
...
if [ -z "${_la}" ]; then
    ...
    bad "ISO 缺已编译 zh_CN/zh_TW locale(locale-gen 可能 abort,中文将回退 C)"
fi
...
for pat in "${TG_TOKEN:-}"; do
  [ -n "${pat}" ] || continue
  hit="$(grep -rlIF "${pat}" ${SCAN_DIRS} 2>/dev/null | head -3)"
  [ -n "${hit}" ] && { bad "ISO 内含敏感串(config.env 的 Telegram 凭据)于：${hit}"; LEAK=1; }
done
...
[ "${LEAK}" = 0 ] && ok "ISO 内未发现 Telegram 凭据/config.env/私钥"

echo
echo "===== 验证结果：通过 ${PASS} / 警告 ${WARN} / 关键失败 ${CRIT} ====="
if [ "${CRIT}" -gt 0 ]; then echo "[错误] 有 ${CRIT} 项关键：缺失 → 拦截，禁止上线"; exit 2
