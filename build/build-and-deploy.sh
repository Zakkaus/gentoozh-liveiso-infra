...
# 预检：开始前几秒查清硬性前置，失败即 fail，不浪费几小时编译。
# git 仓库可达性，3 次退避重试（瞬时 TLS/DNS/5xx 故障不该中止整轮）。
git_reachable() {
    local n=0
...
# 镜像站是唯一发布目标：目标可 ssh、落地目录可写。缺料就不必编译。
preflight_mirror() {
...
update_source() {
    ...
    # 源码新鲜度提醒：本地 != 远端（fetch 可能失败、用了旧副本）只 WARN，不中止整轮
    local remote_head
...
publish_site() {
    ...
    # 对外核对：公开域名服务的就是本轮产物。不一致时留在原地由下一轮覆盖，不删。
...
# 7. 落地页是 Worker 读镜像站列表的视图，有边缘缓存滞后，因此只探测不中止整轮，
# 状态并进末尾的成功通知。
check_landing() {
    local i found=0
