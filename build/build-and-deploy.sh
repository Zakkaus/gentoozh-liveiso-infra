...
. "${CONFIG_ENV}"
# R2 是唯一发布目标;R2_*(token/bucket/endpoint)在 preflight_r2() 里校验。
# 旧的美国下载站(M_*/KEEP)已退役,不再读取、不再要求。
...
# 走到这步:R2 公开域 content-length 已逐字节核对一致(不一致在上面已 fail 退出),如实写"已核对"
VTXT="已上线 R2 并通过对外核对(sha ${SHA:0:12}…)"
