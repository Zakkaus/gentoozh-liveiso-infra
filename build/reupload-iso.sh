#!/bin/bash
# 把 last-iso/ 中已验证的 ISO 重传到镜像站，用于自动构建上传失败后的恢复，无需重新构建。
#
# 以 last-iso/BUILD_MANIFEST 为唯一权威身份，构建脚本 stage 成功时原子写入。
# 无 manifest、实盘 sha 与 manifest 不符、RUN_STAMP 陈旧，一律拒绝重传，
# 避免推送崩在 stage 之前残留的旧产物。
# 异常一律推送 Telegram,与构建脚本共用 TG_TOKEN/TG_CHAT。
set -uo pipefail
...
# 手动执行时 Ctrl-C 或挂断也要让 on_exit 读到真实 rc 并发 FAILED,否则 $? 读成 0 而漏发
trap 'exit 143' TERM; trap 'exit 130' INT; trap 'exit 129' HUP
trap on_exit EXIT

# 与自动构建共用同一把锁：构建执行期间不重传，避免互删旧产物与并发上传
exec 9>"$LOCK"
flock -n 9 || { echo "[错误] 已有构建或重传在执行($LOCK 被占),退出"; DONE=1; exit 0; }

# 镜像站是唯一发布目标，缺 MIRROR_SSH_TARGET 直接拒绝
[ -n "${MIRROR_SSH_TARGET}" ] \
...
# 实盘 sha256 必须与 manifest 对账，不采信同目录的 .sha256,因为旧产物配旧 sha 也自洽
...
# 陈旧闸:RUN_STAMP 超过 14 天需显式 --force 或 CONFIRM=1
d="${STAMP%-*}"; t="${STAMP#*-}"
...
if [ "$ST_EPOCH" != 0 ]; then
  ...
  if [ "$AGE_DAYS" -gt 14 ] && [ "${1:-}" != "--force" ] && [ "${CONFIRM:-}" != 1 ]; then
    echo "[警告] 这次 stamp=$STAMP 已 ${AGE_DAYS} 天，疑似陈旧。确认要重传请:CONFIRM=1 $0  或  $0 --force"
...
# 核对公开域名实际服务的就是本次产物。curl -f 使非 200 直接失败，PUB_LEN 取空即判失败，
# 因为 404 错误页同样带 content-length。
...
# 保留最近 MIRROR_KEEP 份，删除更旧的；本次产物永不删除
...
DONE=1   # 置于收尾展示之前，避免展示用的 ls 瞬时失败误触发 on_exit
